...
class GatewayRunner:
    ...
    async def start(self) -> bool:
        ...
        # Check for pending restart recovery
        await self._recover_from_restart(self)
    ...
    async def _recover_from_restart(self, gateway) -> None:
        from tools.restart_gateway_tool import recover_from_restart
        try:
            await recover_from_restart(gateway)
        except Exception as e:
            logger.exception("Restart recovery failed")
            ...
            if pending:
                ...
                if not was_interrupted:
                    ...
                    if first_response and not _already_streamed:
                        try:
                            await adapter.send(source.chat_id, first_response, metadata=getattr(source, "metadata", None))
                        except Exception as e:
